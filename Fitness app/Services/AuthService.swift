...
class AuthService: ObservableObject{
        ...
        Result<AppUser, Error>) -> Void){
            Auth.auth().createUser(withEmail: email, password: password) { result, error in
                ...
                guard let user = result?.user else{
                    return completion(.failure(SimpleError(message: "Unable to create the user")))// pass ur own custom error
                }
                ...
                do{
                    try self.db.collection("users").document(uid).setData(from: appUser){
                        ...
                        DispatchQueue.main.async {
                            self.currentUser = appUser // update the current user in the main thread
                        }
                ...
                self.fetchCurrentAppUser { res in
                    ...
                    case .success(let appUserObj):
                        if let appUser = appUserObj{
                            ...
                            completion(.failure(SimpleError(message: "Unable to fetch User Details")))
                        }
    ...
    func fetchCurrentAppUser(completion: @escaping (Result<AppUser?, Error>)-> Void){
        ...
        db.collection("users").document(uid).getDocument(){snap, error in
            ...
            do{
                // destruct the stream of data to appuser
    ...
    func updateProfile(displayName: String, completion: @escaping(Result<Void, Error>)-> Void){
        guard let uid = Auth.auth().currentUser?.uid else{
            return completion(.failure(SimpleError(message: "Unable to fetch User details")))
        }
